...
def entrypoint(debug=""):
    ...
    if "rtdetr" in stem:  # guess architecture
        ValueError(f"RTDETR is not supported. Please use official ultralytics")
    elif "fastsam" in stem:
        ValueError(f"FASTSAM is not supported. Please use official ultralytics")
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        ValueError(f"SAM is not supported. Please use official ultralytics")
    elif "cym" in stem:
        from ultralytics import CYM
        model = CYM(model, task='segment')
    else:
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        overrides["source"] = ("https://ultralytics.com/images/boats.jpg"
                               if task == "obb" else DEFAULT_CFG.source or ASSETS)
