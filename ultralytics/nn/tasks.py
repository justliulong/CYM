...
class WorldModel(DetectionModel):
    ...
    def set_classes(self, text, batch=80, cache_clip_model=True):
        ...
        if (not getattr(self, "clip_model", None)
                and cache_clip_model):  # for backwards compatibility of models lacking clip_model attribute
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C2fAttn:
                ...
                args[2] = int(max(round(min(args[2], max_channels // 2 // 32)) *
                                  width, 1) if args[2] > 1 else args[2])  # num heads
            ...
            '''
            actually, in my study, the m must be Segment
            '''
